...
export class RadioERP1 extends ESP3Packet {
  ...
  set subTelNum (v) {
    this.optionalData = ByteArray.from(this.optionalData).setValue(v, 0, 8)
  }
  ...
  set RSSI (v) {
    this.optionalData = ByteArray.from(this.optionalData).setValue(v, 40, 8)
  }
  ...
  set securityLevel (v) {
    this.optionalData = ByteArray.from(this.optionalData).setValue(v, 6 * 8, 8)
  }
  ...
  encode (data, options) {
    ...
    const { payload, status } = encodeData(data, opt)
  ...
  static from (input) {
    ...
    if (Object.prototype.hasOwnProperty.call(input, 'payload')) {
      ...
      const id = typeof input.id === 'number' ? ByteArray.create(8).setValue(input.id, 0, 64).toString() : ByteArray.from(input.id || '00000000').toString()
      return RadioERP1.from({
        data: [input.rorg || rorg, pl, id, input.status || 0],
