...
export function encodeData (data, options) {
  ...
  if (eep) {
    const desc = eep.case.find(desc => {
      ...
      return setDataFields(scratchPayload, desc, data) && satisfiesCondition(desc, { ...selector, payload: scratchPayload, status: status })
    })
    if (desc) {
      const payloadLength = getPayloadLengthFromEEPDescriptor(desc)
      const payload = selector.payload || ByteArray.create(payloadLength)
      adjustPayloadLength(payload, payloadLength)
      setDataFields(payload, desc, data)
      const status = desc.condition && desc.condition.statusfield ? createStatusByte(desc.condition.statusfield, ByteArray.from(0))[0] : 0
