...
function createStatusByte (fields, status) {
  if (!status) status = ByteArray.from(0)
...
function findEEPCase (options) {
  var selector = { ...{ direction: 1, status: 0 }, ...options }
  var eep = getEEP(selector.eep.toLowerCase())
  let eepCase = eep.case.find(function (item) {
    return getPayloadLengthFromEEPDescriptor(item) >= selector.payload.length && satisfiesCondition(item, selector)
  });
  if (!eepCase){
    throw new Error('Could not determine case for ' + JSON.stringify(options))
  }
  return { eep, eepCase }
}

function satisfiesCondition (desc, selector) {
  switch (getConditionType(desc)) {
    case 'status':
      var itemStatus = createStatusByte(desc.condition.statusfield)
      if (itemStatus[0] === selector.status) {
        return true
      }
      break
    case 'data':
      var cond = selector.payload.getValue(parseInt(desc.condition.datafield.bitoffs), parseInt(desc.condition.datafield.bitsize))
      if (parseInt(desc.condition.datafield.value) === cond) {
        return true
      }
      break
    case 'direction':
      if (parseInt(desc.condition.direction) === selector.direction) {
        return true
      }
      break
    case 'none':
...
export function decodeAll(radio, eep, direction = 1){
  const eepResult = findEEPCase({
    eep: eep,
    direction: direction,
    status: radio.status,
    payload: radio.payload
  })
  const value = decodeCase(radio.payload, eepResult.eepCase)
  return { eep: eepResult.eep, eepCase: eepResult.eepCase, data: value }
}

export function decode (radio, eep, direction = 1) {
  return Object.freeze(decodeAll(radio, eep, direction).data);
}

function decodeCase (tel, c) {
  try {
    return arrayOrSingle(c.datafield).reduce(makeFieldExtractor(tel), {})
  } catch (err) {
...
function getDataFieldByShortcut (datafield, shortcut) {
  return arrayOrSingle(datafield).find(item => item.shortcut === shortcut)
}

function extractEnumValue (value, list) {
  return arrayOrSingle(list).find(item => {
    if ('min' in item) {
...
function setRangeValue (rawValue, datafield, eep, payload) {
  ...
  if ('ref' in scale) {
    scale = getRefField(scale.ref, eep, payload).scale
  }
  if ('ref' in range) {
    range = getRefField(range.ref, eep, payload).range
  }
...
function getPayloadLengthFromEEPDescriptor (desc) {
  ...
  var res = arrayOrSingle(desc.datafield).map(field => {
    if ('spread' in field) {
  ...
  return Math.ceil((parseInt(getBitoffs(res)) + parseInt(res.bitsize)) / 8)
}

function setDataFields (payload, desc, data) {
  for (var name in data) {
    const field = getDataFieldByShortcut(desc.datafield, name)
    if (field) {
      const fieldType = getFieldType(field)
      const rawValue = Object.prototype.hasOwnProperty.call(data[name], 'rawValue') ? data[name].rawValue : undefined
      const value = Object.prototype.hasOwnProperty.call(data[name], 'value') ? data[name].value : undefined
      ...
      if (typeof val === 'number') {
        if (fieldType === 'range' || fieldType === 'spread') {
          ...
          if (fieldType === 'spread') {
            var totalLength = field.spread.reduce((acc, current) => acc + parseInt(current.bitsize), 0)
        ...
        payload.setValue(rawValue, parseInt(field.bitoffs), parseInt(field.bitsize))
      }
    } else {
      return false
    }
...
function adjustPayloadLength (payload, payloadLength) {
...
export function encodeData (data, options) {
  var selector = { ...{ eep: 'f6-01-01', direction: 1, status: 0 }, ...options }
  let eepCase
  if (selector.eepCase){
    eepCase = selector.eepCase
  } else {
    var eep = getEEP(selector.eep)
    if (eep) {
      eepCase = eep.case.find(desc => {
        const length = getPayloadLengthFromEEPDescriptor(desc)
        var scratchPayload = ByteArray.create(length)

        const status = desc.condition && desc.condition.statusfield ? createStatusByte(desc.condition.statusfield, ByteArray.from(selector.status))[0] : 0

        return setDataFields(scratchPayload, desc, data) && satisfiesCondition(desc, {
          ...selector,
  ...
  if (eepCase) {
    const payloadLengthFromEEPDescriptor = getPayloadLengthFromEEPDescriptor(eepCase);
    const payload = selector.payload || ByteArray.create(payloadLengthFromEEPDescriptor)
    adjustPayloadLength(payload, payloadLengthFromEEPDescriptor)
    setDataFields(payload, eepCase, data)
    const status = eepCase.condition && eepCase.condition.statusfield ? createStatusByte(eepCase.condition.statusfield, ByteArray.from(0))[0] : 0
    return { payload, status }
  }
  throw new Error('No matching EEP and case found!')
}
